...
/**
 * @title SmartSentinelsGenesis
 * @author Andrei
 * @notice The SmartSentinels Genesis NFT collection.
...
contract SmartSentinelsGenesis is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Allows a user to mint one NFT by paying the designated mint price.
     * @dev This function checks for public minting status, token supply limits, and correct payment before minting a token and distributing the funds.
     */
    function publicMint() external payable nonReentrant {
        ...
        (bool ok1, ) = PAYEE1.call{value: (totalReceived * 15) / 100}("");
        require(ok1, "Payout1 failed");
        (bool ok2, ) = PAYEE2.call{value: totalReceived / 50}("");
        require(ok2, "Payout2 failed");
        (bool ok3, ) = PAYEE3.call{value: totalReceived / 10}("");
        require(ok3, "Payout3 failed");
        (bool ok4, ) = PAYEE4.call{value: totalReceived / 50}("");
        require(ok4, "Payout4 failed");
        (bool ok5, ) = PAYEE5.call{value: totalReceived / 10}("");
        require(ok5, "Payout5 failed");
        (bool ok6, ) = PAYEE6.call{value: (totalReceived * 61) / 100}("");
        require(ok6, "Payout6 failed");
     ...
     */
    function setBaseURI(string calldata newBase) external onlyOwner payable {
        if (baseURIFrozen) {
     ...
     */
    function freezeBaseURI() external onlyOwner payable {
        if (!baseURIFrozen) {
            baseURIFrozen = true;
            emit BaseURIFrozen();
        }
     ...
     */
    function setContractURI(string calldata newContractURI) external onlyOwner payable {
     ...
     */
    function setMediaBaseURI(string calldata newMediaBaseURI) external onlyOwner payable {
     ...
     */
    function setMediaExtension(string calldata newExt) external onlyOwner payable {
     ...
     */
    function setPublicMintEnabled(bool enabled) external onlyOwner payable {
     ...
     * @dev Checks if the new price is different from the current one before updating.
     * @param newPrice The new mint price in wei.
     */
    function setMintPrice(uint256 newPrice) external onlyOwner payable {
        if (mintPrice != newPrice) {
            mintPrice = newPrice;
    ...
    /**
     * @notice Withdraws any leftover funds from the contract.
